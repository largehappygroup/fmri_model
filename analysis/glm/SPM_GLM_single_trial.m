...
function SPM_GLM_single_trial(subID, fmri_datapath, outpath, onsets, task)
...
% temporal resolution of the 
    ...
    %     onsets = onsets.Var1;
    % end
    num_trials = size(onsets,2);

    for i=1:num_trials
        matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(1).cond(i).name     = char(i);
        matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(1).cond(i).onset    = onsets(:,i);
