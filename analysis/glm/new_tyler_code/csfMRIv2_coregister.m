...
% with 'ss-mt1' (skull-stripped, bias-corrected T1).
    ...
    switch hiresSeq
        case 'spgr'
            matlabbatch{1}.spm.spatial.coreg.estimate.ref = cellstr(strcat([pwd '/' wrw '/anatomy/t1spgr_208sl/'], spm_select('List', [pwd '/' wrw '/anatomy/t1spgr_208sl/'], '^ss-mt1*.*nii$')));
        case 'flair'
            matlabbatch{1}.spm.spatial.coreg.estimate.ref = cellstr(strcat([pwd '/' wrw '/anatomy/t1overlay_60sl/'], spm_select('List', [pwd '/' wrw '/anatomy/t1overlay_60sl/'], '^ss-mt1*.*nii$')));
    end
...
% parameters can be added to those image headers.
    ...
    matlabbatch{1}.spm.spatial.coreg.estimate.source = cellstr(strcat([pwd '/' wrw '/func/mean/'], spm_select('List', [pwd '/' wrw '/func/mean/'], '^meanurun*.*nii$')));
    
    allEPI = cellstr(strcat([pwd '/' wrw '/func/rest/run_01/'], spm_select('List', [pwd '/' wrw '/func/rest/run_01/'], '^urun_01.*nii$')));
    
    for iRun = 1:nRun
        
        scans  = cellstr(strcat([pwd '/' wrw '/func/typing/run_0' num2str(iRun) '/'], spm_select('List', [pwd '/' wrw '/func/typing/run_0' num2str(iRun) '/'], ['^urun_0*.*nii$'])));
