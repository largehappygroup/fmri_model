...
%--------------------------------------------------------------------------
    ...
    wr = dir('wrw*');
    wrw = wr(1).name;
    allEPI = cellstr(strcat([pwd '/' wrw '/func/rest/run_01/'], spm_select('List', [pwd '/' wrw '/func/rest/run_01/'], '^wurun*.*nii$')));
    
    for iRun = 1:nRun
        
        scans  = cellstr(strcat([pwd '/' wrw '/func/typing/run_0' num2str(iRun) '/'], spm_select('List', [pwd '/' wrw '/func/typing/run_0' num2str(iRun) '/'], '^wurun*.*nii$')));
        
        allEPI = [allEPI; scans];
        
    end
    
    meanScan = cellstr(strcat([pwd '/' wrw '/func/mean/'], spm_select('List', [pwd '/' wrw '/func/mean'], '^wmeanu*.*nii$')));
