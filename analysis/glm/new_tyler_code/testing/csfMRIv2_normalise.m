...
switch hiresSeq
        case 'spgr'
            matlabbatch{1}.spm.spatial.normalise.write.subj.def = cellstr(strcat([pwd '/anatomy/t1spgr_208sl/'], spm_select('List', [pwd '/anatomy/t1spgr_208sl/'], '^y_t1*.*nii$')));
            matlabbatch{1}.spm.spatial.normalise.write.subj.resample = cellstr(strcat([pwd '/anatomy/t1spgr_208sl/'], spm_select('List', [pwd '/anatomy/t1spgr_208sl/'], '^ss-mt1*.*nii$')));
        case 'flair'
            matlabbatch{1}.spm.spatial.normalise.write.subj.def = cellstr(strcat([pwd '/anatomy/t1overlay_60sl/'], spm_select('List', [pwd '/anatomy/t1overlay_60sl/'], '^y_t1*.*nii$')));
            matlabbatch{1}.spm.spatial.normalise.write.subj.resample = cellstr(strcat([pwd '/anatomy/t1overlay_60sl/'], spm_select('List', [pwd '/anatomy/t1overlay_60sl/'], '^ss-mt1*.*nii$')));
    end
...
%--------------------------------------------------------------------------
    ...
    allEPI = cellstr(strcat([pwd '/func/rest/run_01/'], spm_select('List', [pwd '/func/rest/run_01/'], '^urun*.*nii$')));
    
    for iRun = 1:nRun
        
        scans  = cellstr(strcat([pwd '/func/reviewblock/run_0' num2str(iRun) '/'], spm_select('List', [pwd '/func/reviewblock/run_0' num2str(iRun) '/'], '^urun*.*nii$')));
        
        allEPI = [allEPI; scans];
        
    end
    
    meanScan = cellstr(strcat([pwd 'func/mean/'], spm_select('List', [pwd 'func/mean/'], '^meanu*.*nii$')));
