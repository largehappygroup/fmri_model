...
%--------------------------------------------------------------------------
%hiresPath = ['/anatomy/t1' hiresSeq '*'];
    hiresPath = [''];
    switch hiresSeq
        case 'spgr'
            hiresPath = ['./anatomy/t1spgr_208sl']
            matlabbatch{1}.spm.spatial.preproc.channel.vols = cellstr(strcat([hiresPath '/'], spm_select('List', hiresPath, ['t1spgr_208sl.nii'])));
        case 'flair'
            hiresPath = ['./anatomy/t1overlay_60sl']
            matlabbatch{1}.spm.spatial.preproc.channel.vols = cellstr(strcat([hiresPath '/'], spm_select('List', hiresPath, ['ht1overlay_60sl.nii'])));
    end
...
%--------------------------------------------------------------------------
    ...
    gm  = cellstr(strcat([hiresPath '/'], spm_select('List', hiresPath, '^c1.*nii$')));
    wm  = cellstr(strcat([hiresPath '/'], spm_select('List', hiresPath, '^c2.*nii$')));
    csf = cellstr(strcat([hiresPath '/'], spm_select('List', hiresPath, '^c3.*nii$')));
